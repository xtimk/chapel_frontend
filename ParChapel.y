-- This Happy file was machine-generated by the BNF converter
{
{-# OPTIONS_GHC -fno-warn-incomplete-patterns -fno-warn-overlapping-patterns #-}
module ParChapel where
import AbsChapel
import LexChapel
import ErrM

}

%name pProgram Program
-- no lexer declaration
%monad { Err } { thenM } { returnM }
%tokentype {Token}
%token
  '!=' { PT _ (TS _ 1) }
  '%' { PT _ (TS _ 2) }
  '&' { PT _ (TS _ 3) }
  '&&' { PT _ (TS _ 4) }
  '(' { PT _ (TS _ 5) }
  ')' { PT _ (TS _ 6) }
  '*' { PT _ (TS _ 7) }
  '+' { PT _ (TS _ 8) }
  '+=' { PT _ (TS _ 9) }
  ',' { PT _ (TS _ 10) }
  '-' { PT _ (TS _ 11) }
  '/' { PT _ (TS _ 12) }
  ';' { PT _ (TS _ 13) }
  '<' { PT _ (TS _ 14) }
  '<<' { PT _ (TS _ 15) }
  '<=' { PT _ (TS _ 16) }
  '=' { PT _ (TS _ 17) }
  '==' { PT _ (TS _ 18) }
  '>' { PT _ (TS _ 19) }
  '>=' { PT _ (TS _ 20) }
  '>>' { PT _ (TS _ 21) }
  '^' { PT _ (TS _ 22) }
  'do' { PT _ (TS _ 23) }
  'int' { PT _ (TS _ 24) }
  'ref' { PT _ (TS _ 25) }
  'while' { PT _ (TS _ 26) }
  '{' { PT _ (TS _ 27) }
  '|' { PT _ (TS _ 28) }
  '||' { PT _ (TS _ 29) }
  '}' { PT _ (TS _ 30) }
  L_ident  { PT _ (TV $$) }
  L_quoted { PT _ (TL $$) }
  L_doubl  { PT _ (TD $$) }
  L_charac { PT _ (TC $$) }
  L_integ  { PT _ (TI $$) }

%%

Ident   :: { Ident }
Ident    : L_ident  { Ident $1 }

String  :: { String }
String   : L_quoted {  $1 }

Double  :: { Double }
Double   : L_doubl  { (read ( $1)) :: Double }

Char    :: { Char }
Char     : L_charac { (read ( $1)) :: Char }

Integer :: { Integer }
Integer  : L_integ  { (read ( $1)) :: Integer }

Program :: { Program }
Program : Module { AbsChapel.Progr $1 }
Module :: { Module }
Module : ListExt { AbsChapel.Mod $1 }
ListExt :: { [Ext] }
ListExt : Ext { (:[]) $1 } | Ext ListExt { (:) $1 $2 }
Ext :: { Ext }
Ext : Declaration { AbsChapel.ExtDecl $1 }
    | Function { AbsChapel.ExtFun $1 }
Declaration :: { Declaration }
Declaration : Type Id ';' { AbsChapel.NoAssgmDec $1 $2 }
            | Type Id AssgnmOp Exp ';' { AbsChapel.AssgmDec $1 $2 $3 $4 }
Function :: { Function }
Function : Signature Body { AbsChapel.FunDec $1 $2 }
Signature :: { Signature }
Signature : Type Id FunctionParams { AbsChapel.Sign $1 $2 $3 }
FunctionParams :: { FunctionParams }
FunctionParams : '(' ListParam ')' { AbsChapel.FunParams $2 }
ListParam :: { [Param] }
ListParam : {- empty -} { [] }
          | Param { (:[]) $1 }
          | Param ',' ListParam { (:) $1 $3 }
Param :: { Param }
Param : Type Id { AbsChapel.ParNoMode $1 $2 }
      | Mode Type Id { AbsChapel.ParWMode $1 $2 $3 }
Body :: { Body }
Body : '{' ListBodyStatement '}' { AbsChapel.FunBlock (reverse $2) }
ListBodyStatement :: { [BodyStatement] }
ListBodyStatement : {- empty -} { [] }
                  | ListBodyStatement BodyStatement { flip (:) $1 $2 }
BodyStatement :: { BodyStatement }
BodyStatement : Statement { AbsChapel.Stm $1 }
              | Function ';' { AbsChapel.Fun $1 }
              | Declaration { AbsChapel.Decl $1 }
              | Body { AbsChapel.Block $1 }
Statement :: { Statement }
Statement : 'do' 'while' Body Guard { AbsChapel.DoWhile $3 $4 }
Guard :: { Guard }
Guard : '(' Exp ')' { AbsChapel.SGuard $2 }
Type :: { Type }
Type : 'int' { AbsChapel.Tint }
Id :: { Id }
Id : Ident { AbsChapel.Tid $1 }
AssgnmOp :: { AssgnmOp }
AssgnmOp : '=' { AbsChapel.AssgnEq } | '+=' { AbsChapel.AssgnPlEq }
Mode :: { Mode }
Mode : 'ref' { AbsChapel.RefMode }
Exp :: { Exp }
Exp : Exp '||' Exp5 { AbsChapel.Elor $1 $3 } | Exp5 { $1 }
Exp5 :: { Exp }
Exp5 : Exp5 '&&' Exp6 { AbsChapel.Eland $1 $3 } | Exp6 { $1 }
Exp6 :: { Exp }
Exp6 : Exp6 '|' Exp7 { AbsChapel.Ebitor $1 $3 } | Exp7 { $1 }
Exp7 :: { Exp }
Exp7 : Exp7 '^' Exp8 { AbsChapel.Ebitexor $1 $3 } | Exp8 { $1 }
Exp8 :: { Exp }
Exp8 : Exp8 '&' Exp9 { AbsChapel.Ebitand $1 $3 } | Exp9 { $1 }
Exp9 :: { Exp }
Exp9 : Exp9 '==' Exp10 { AbsChapel.Eeq $1 $3 }
     | Exp9 '!=' Exp10 { AbsChapel.Eneq $1 $3 }
     | Exp10 { $1 }
Exp10 :: { Exp }
Exp10 : Exp10 '<' Exp11 { AbsChapel.Elthen $1 $3 }
      | Exp10 '>' Exp11 { AbsChapel.Egrthen $1 $3 }
      | Exp10 '<=' Exp11 { AbsChapel.Ele $1 $3 }
      | Exp10 '>=' Exp11 { AbsChapel.Ege $1 $3 }
      | Exp11 { $1 }
Exp11 :: { Exp }
Exp11 : Exp11 '<<' Exp12 { AbsChapel.Eleft $1 $3 }
      | Exp11 '>>' Exp12 { AbsChapel.Eright $1 $3 }
      | Exp12 { $1 }
Exp12 :: { Exp }
Exp12 : Exp12 '+' Exp13 { AbsChapel.Eplus $1 $3 }
      | Exp12 '-' Exp13 { AbsChapel.Eminus $1 $3 }
      | Exp13 { $1 }
Exp13 :: { Exp }
Exp13 : Exp13 '*' Exp14 { AbsChapel.Etimes $1 $3 }
      | Exp13 '/' Exp14 { AbsChapel.Ediv $1 $3 }
      | Exp13 '%' Exp14 { AbsChapel.Emod $1 $3 }
      | Exp14 { $1 }
Exp14 :: { Exp }
Exp14 : '(' Exp ')' { $2 }
      | Ident { AbsChapel.Evar $1 }
      | Constant { AbsChapel.Econst $1 }
      | String { AbsChapel.Estring $1 }
Constant :: { Constant }
Constant : Double { AbsChapel.Efloat $1 }
         | Char { AbsChapel.Echar $1 }
         | Integer { AbsChapel.Eint $1 }
{

returnM :: a -> Err a
returnM = return

thenM :: Err a -> (a -> Err b) -> Err b
thenM = (>>=)

happyError :: [Token] -> Err a
happyError ts =
  Bad $ "syntax error at " ++ tokenPos ts ++
  case ts of
    []      -> []
    [Err _] -> " due to lexer error"
    t:_     -> " before `" ++ id(prToken t) ++ "'"

myLexer = tokens
}

